...
$define({
  ...
  events: {
    "tableView:didSelectRowAtIndexPath:": function(table, indexPath) {
      table.invoke("deselectRowAtIndexPath:animated:", indexPath, true)
    ...
    "tableView:editActionsForRowAtIndexPath:": function(table, indexPath) {
      var isHighlighted = table.invoke("cellForRowAtIndexPath", indexPath).invoke("isHighlighted")
      ...
      var action = $objc("UITableViewRowAction").invoke("rowActionWithStyle:title:handler", 1, _lan.l10n("more"), handler)
      action.invoke("setBackgroundColor", $color("#EFF1F1"))
      return [action]
    },
    "tableView:didHighlightRowAtIndexPath:": function(table, indexPath) {
      $ui.animate({
        duration: 0.3,
        animation: function() {
          table.rawValue().cell(indexPath.rawValue()).scale(0.9)
        }
      })
    },
    ...
    "tableView:willDisplayCell:forRowAtIndexPath:": function(table, cell, indexPath) {
      if (cell.invoke("selectionStyle")) {
        cell.invoke("setSelectionStyle", 0)
      }
    },
    "scrollViewDidEndDragging:willDecelerate:": function(scroll, decelerate) {
      var offset = scroll.invoke("contentOffset").y
      if (decelerate === false) {
        var top = offset > 0 && offset <= 25 ? 0 : (offset > 25 && offset < 50 ? 50 : null)
        if (top == null) return
        scroll.invoke("setContentOffset:animated:", $point(0, top), true)
      } else {
        if (offset < -100) {
          _data.search()
        }
    ...
    "scrollViewShouldScrollToTop:": function(scroll) {
      scroll.invoke("setContentOffset:animated:", $point(0, 50), true)
    }
  }
})
var delegate = $objc("MainTableViewDelegate").invoke("alloc.init")
...
function render() {
  ...
  $("MAIN").runtimeValue().invoke("setDelegate", delegate)
}
